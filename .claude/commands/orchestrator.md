...
### Monitoring Cadence
- Check each sub-repo's mail immediately after dispatching to confirm acknowledgment.
- **Wait 5 minutes** before the first status check — coordinators need time to boot, read the dispatch, and spawn their first agents.
- After the first check, **wait 7 minutes** between subsequent checks. Agents typically need 5-15 minutes per issue depending on complexity.
- Use `sleep <seconds>` between checks. Do not poll in tight loops — each check costs tokens.
- Check all sub-repos in a single batch (parallel tool calls), not one at a time.
- When a coordinator is on its final issue, you may shorten the interval to 5 minutes.
- Stop a coordinator as soon as it sends a `result` mail — do not wait for other repos to finish.
